...
import { fetchFinancialData, addComment, updateComment, fetchClients, Client } from '../services/api';
...
const FinancialTable: React.FC = () => {
  ...
  const groupFinancialData = (data: FinancialData[]): GroupedFinancialData[] => {
    ...
    Object.values(groupMap).forEach(group => {
      group.items.sort((a, b) => a.ordering_id.localeCompare(b.ordering_id));
    });
  ...
  const loadFinancialData = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await fetchFinancialData(selectedMonth, selectedClient);
      setFinancialData(data.data);
      setTotalActual(data.totalActual);
      setTotalBudget(data.totalBudget);
      
      // Group the data
      const grouped = groupFinancialData(data.data);
  ...
  const handleAddComment = async () => {
    try {
      ...
      // Always create a new comment record regardless if it's an edit or new comment
      await addComment(selectedEntry, commentText, userName, selectedClient);
